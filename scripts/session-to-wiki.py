#!/usr/bin/env python3
"""session-to-wiki.py (v7) — Extract durable facts from Hermes Agent sessions to OKF wiki pages.
...
v7 changes (over v6) — slug uniqueness + staging auto-lifecycle:
  1. SLUG REGISTRY: .slug_registry.json maps slug → canonical (title, type, path).
     write_fact() checks the registry before creating a new page and redirects to
     the canonical page if a slug already exists in another directory.
  2. TYPE CONFLICT RESOLUTION: When the LLM returns the same entity with a
     different type, specificity ranking (entity > concept > decision/comparison
     > question) determines which type wins. More specific incoming types upgrade
     the canonical type.
  3. EXTRACTION PASS MERGE KEY CHANGED from (type, title_lower) to title_lower
     only, preventing same-title/different-type from creating separate entries.
  4. STAGING AUTO-PROMOTE: Pages with ≥ STAGING_AUTO_PROMOTE_THRESHOLD distinct
     sources are automatically promoted to live directories at run end.
  5. STAGING AUTO-PURGE: Pages older than STAGING_MAX_AGE_DAYS with ≤ 1 source
     and no index references are moved to .trash/staging/ at run end.
  6. BACKLINK SAFETY: Auto-purge checks index.md for title references before
     removing — anything linked from live pages is preserved.
  7. REGISTRY REBUILD: load_slug_registry() auto-recovers from corruption by
     scanning all live wiki pages and rebuilding from frontmatter.
...
# -- v7: Slug registry + staging auto-lifecycle ------------------------------
REGISTRY_FILE = WIKI_DIR / ".slug_registry.json"
TYPE_SPECIFICITY = {"entity": 4, "concept": 3, "decision": 2, "comparison": 2, "question": 1}
STAGING_PROMOTE_THRESHOLD = int(os.environ.get("STAGING_AUTO_PROMOTE_THRESHOLD", "3"))
STAGING_MAX_AGE_DAYS = int(os.environ.get("STAGING_MAX_AGE_DAYS", "14"))
STAGING_AUTO_PROMOTE = os.environ.get("STAGING_AUTO_PROMOTE", "1") not in ("0", "no", "false")
STAGING_AUTO_PURGE = os.environ.get("STAGING_AUTO_PURGE", "1") not in ("0", "no", "false")
...
# -- Slug registry (v7) -------------------------------------------------------
# The registry is the single source of truth for which slug maps to which
# file in which directory. It prevents duplicate slugs across live directories.

def load_slug_registry() -> dict:
    """Return {slug: {title, type, path, created, updated, aliases}}."""
    ...
    try:
        ...
        log(f"Registry file corrupt ({e}) — rebuilding from disk", "WARN")
        return rebuild_slug_registry()


def save_slug_registry(registry: dict):
    """Atomic write via tempfile to avoid partial-write corruption."""
    import tempfile
    tmp = tempfile.NamedTemporaryFile(mode="w", dir=str(REGISTRY_FILE.parent),
                                      prefix=".slug_registry.", delete=False,
                                      encoding="utf-8")
    try:
        ...
        os.fsync(tmp.fileno())
        tmp.close()
        os.replace(tmp.name, str(REGISTRY_FILE))
    except Exception:
        try:
            os.unlink(tmp.name)
        except OSError:
...
def rebuild_slug_registry() -> dict:
    """Scan all live wiki pages and rebuild the registry from frontmatter."""
    registry = {"version": 1, "slugs": {}}
    for subdir in LIVE_DIRS:
        d = WIKI_DIR / subdir
        if not d.exists():
            continue
        for f in sorted(d.iterdir()):
            ...
            title = fm.get("title", "").strip() if fm else ""
            if not title:
                continue
            slug_name = safe_filename(title)
            relpath = str(f.relative_to(WIKI_DIR))
            registry["slugs"][slug_name] = {
                "title": title,
...
def register_slug(slug_name: str, title: str, ftype: str,
                  relpath: str, registry: dict, aliases: list = None):
    """Register a new slug. Raises ValueError if slug already exists."""
    if slug_name in registry["slugs"]:
        raise ValueError(f"Slug '{slug_name}' already registered -> "
                         f"{registry['slugs'][slug_name]['path']}")
    registry["slugs"][slug_name] = {
        "title": title,
        "type": ftype,
        "path": relpath,
...
def update_registry_entry(slug_name: str, registry: dict, **kwargs):
    """Update fields on an existing registry entry."""
    if slug_name not in registry["slugs"]:
        return
    entry = registry["slugs"][slug_name]
...
def get_canonical_title(title: str, registry: dict):
    """Return (canonical_title, canonical_path, canonical_type, is_match)
    or (None, None, None, False) if not found.
    
    Checks: 1) direct slug match  2) title match  3) alias match
    """
    slug_candidate = safe_filename(title)
    slugs = registry.get("slugs", {})
    
    # Direct slug match
    if slug_candidate in slugs:
        e = slugs[slug_candidate]
        return (e["title"], e["path"], e.get("type"), True)
    
    # Title match (case-insensitive)
    title_lower = title.lower().strip()
    for sname, entry in slugs.items():
        if entry["title"].lower() == title_lower:
            return (entry["title"], entry["path"], entry.get("type"), True)
...
def resolve_type_conflict(incoming_type: str, canonical_type: str) -> str:
    """Return the preferred type when same entity has different types.
    
    Rule: more specific wins. entity > concept > decision/comparison > question.
    """
    inc = TYPE_SPECIFICITY.get(incoming_type, 0)
    can = TYPE_SPECIFICITY.get(canonical_type, 0)
    if inc > can:
...
def call_llm_for_extraction(prompt: str, session_id: str) -> Optional[list]:
    ...
    for attempt in range(1, max_retries + 2):
        try:
            resp = requests.post(url, json=body_dict, headers=headers, timeout=(30, 180))
...
def write_fact(fact: dict, existing_pages: dict, source_session: str = None,
    ...
    # ---- Slug registry redirect (v7) -------------------------------------------
    # Check global slug registry before create: if this title already has a
    # canonical page (different directory, same slug), route to enrichment.
    # This prevents same-slug/different-directory duplicates.
    canonical = None
    if not existing_path:
        registry = load_slug_registry()
        canonical = get_canonical_title(title, registry)
        if canonical[3]:
            # This entity already has a page — redirect to enrichment
            canonical_path = WIKI_DIR / canonical[1]
            if canonical_path.exists():
                existing_path = canonical_path
                log(f"  Registry redirect: '{title}' -> [[{canonical[0]}]] "
                    f"(incoming type={ftype}, canonical type={canonical[2]})")
                # Resolve type: if incoming is more specific, upgrade canonical
                resolved_type = resolve_type_conflict(ftype, canonical[2])
                if resolved_type != canonical[2]:
                    log(f"  Type upgrade: {canonical[2]} -> {resolved_type} for [[{canonical[0]}]]")
    ...
    for i, transcript in enumerate(chunks, 1):
        ...
        for fact in enrich_facts:
            if isinstance(fact, dict) and fact.get("title"):
                # v7: merge by title_lower only (not type+title) to prevent
                # same-title/different-type from creating separate files.
                title_lower_key = fact["title"].strip().lower()
                existing_fact = merged.get(title_lower_key)
                if existing_fact and existing_fact.get("type") != fact.get("type"):
...
def auto_manage_staging(registry: dict):
    """Auto-promote and auto-purge staging/ pages at end of extraction run."""
    if not STAGING_AUTO_PROMOTE and not STAGING_AUTO_PURGE:
        return
    ...
    for f in sorted(d.glob("*.md")):
        if f.stem.endswith("--pending"):
            continue  # never auto-manage pending-facts files
        ...
        distinct_sources = len(set(s for s in sources if s))

        # Auto-promote: ≥ threshold distinct sources
        if STAGING_AUTO_PROMOTE and distinct_sources >= STAGING_PROMOTE_THRESHOLD:
            slug_name = f.stem
            ftype = fm.get("type", "concept")
            if ftype not in FACT_DIRS:
                ftype = "concept"
            dest_dir = WIKI_DIR / FACT_DIRS[ftype]
            dest = dest_dir / f.name

            if dest.exists():
                log(f"  AUTO-PROMOTE SKIP {f.name}: destination exists — merging needed")
                continue
            ...
            dest_dir.mkdir(parents=True, exist_ok=True)
            dest.write_text(rebuild_page_content(fm, body))
            relpath = str(dest.relative_to(WIKI_DIR))

            try:
                register_slug(slug_name, fm.get("title", slug_name), ftype,
                             relpath, registry)
            except ValueError:
                update_registry_entry(slug_name, registry, path=relpath, type=ftype)

            f.unlink()
            promoted.append((fm.get("title", slug_name), ftype))
            log(f"  AUTO-PROMOTE: {f.name} -> {relpath}")
            continue

        # Auto-purge: >14 days old, ≤1 source, no live backlinks
        if (STAGING_AUTO_PURGE and age_days > STAGING_MAX_AGE_DAYS
                and distinct_sources <= 1):
            title = fm.get("title", "")
            if title:
                wikilink_ref = f"[[{title}]]"
                # Quick scan: check index.md for backlinks (much cheaper than
                # scanning all 500+ pages)
                index_text = INDEX_FILE.read_text(encoding="utf-8",
                                                  errors="replace")
                if title in index_text:
                    log(f"  AUTO-PURGE SKIP {f.name}: referenced in index")
                    continue

            trash_dir = WIKI_DIR / ".trash" / "staging"
            trash_dir.mkdir(parents=True, exist_ok=True)
            ts = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
            dest = trash_dir / f"{f.stem}--{ts}.md"
            f.rename(dest)
            purged.append(f.name)
            log(f"  AUTO-PURGE: {f.name} (age {age_days}d, sources={distinct_sources})")

    if promoted:
        update_index_with_entries(promoted)
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        append_log(f"## [{today}] AUTO-PROMOTE | {len(promoted)} pages from staging")
        git_commit(f"auto-promote: {len(promoted)} pages from staging")
    if purged:
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        append_log(f"## [{today}] AUTO-PURGE | {len(purged)} stale pages -> .trash/staging/")
        git_commit(f"auto-purge: {len(purged)} stale staging pages")

    if promoted or purged:
        log(f"Staging auto-manage: {len(promoted)} promoted, {len(purged)} purged")
...
Environment:
  ...
  STAGING_AUTO_PROMOTE_THRESHOLD  Min sources to auto-promote (3)
  STAGING_MAX_AGE_DAYS      Auto-purge staging pages older than N days (14)
  STAGING_AUTO_PROMOTE      Set to 0 to disable auto-promotion (default: 1)
  STAGING_AUTO_PURGE        Set to 0 to disable auto-purge (default: 1)
